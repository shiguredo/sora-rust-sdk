...
- 待機ループの開始前に `command_rx` を閉じ、以後のコマンド送信を拒否する。終了フェーズに入った時点 (run のメインループを抜けた直後) に `command_rx.close()` を呼び、以後の送信は即座に `Error::CommandSendFailed` にする
  ...
  - クローズ前に送信されたコマンドはドレインして処理する。`Disconnect` には ack を返す (呼び出し側は成功する)。`Disconnect` 以外のコマンド (`GetStats` / `GetSelectedSignalingUrl` / `GetConnectedSignalingUrl` / `SendRpcRequest` / `SendMessage`) は破棄する (その呼び出し側は従来どおり `Error::CommandResponseMissing` になる)
  - ドレインは `recv()` を `None` まで回して行う (非同期)。tokio の unbounded channel では `send()` の送信済みメッセージ数の更新 (CAS) とリストへの push がアトミックでなく、CAS 成功直後に送信タスクがプリエンプトされると、その間に `close()` を呼んだ場合に送信済みメッセージ数だけが増えた状態になる。`recv()` は送信済みメッセージ数が 0 になるまで Pending を返すため、`try_recv()` のワンショットドレインでは拾えない in-flight のメッセージも確実に処理できる
...
- 終了フェーズに入った後は `command_rx` が閉じられ、以後の `disconnect()` は ack の代わりに `Error::CommandSendFailed` を返す。クローズ前に送信された `Disconnect` には ack が返る。いずれの場合も `Error::CommandResponseMissing` にはならない
...
- 待機ループの修正を検証するテストがある
  ...
  - 終了フェーズのコマンド拒否 (`close_command_channel_and_ack_pending_disconnects`) を実際の mpsc チャネルを引数で受け取る単体テスト可能な関数に切り出し、`src/connection.rs` 内の `#[cfg(test)]` モジュールで次を検証する
    - クローズ前に送信された `Disconnect` に ack が返ること
    - クローズ後の送信が失敗し、呼び出し側が `Error::CommandSendFailed` になること (チャネルが閉じられているため送信自体が失敗する)
    - クローズ前に送信された `Disconnect` 以外のコマンド (`GetStats` 等) が応答されず、呼び出し側が `Error::CommandResponseMissing` になること
