...
- Release asset は sumomo バイナリのみとする
...
- 必要なら `README.md` への入手手順の短文追記

## 解決方法

`.github/workflows/release.yml` に `build-sumomo` ジョブを追加した。

- `ubuntu-24.04` / `ubuntu-24.04-arm` / `ubuntu-26.04` / `ubuntu-26.04-arm` の 4 行列で `cargo build -p sumomo --release --locked` を実行する
- `ci.yml` と同じ apt 依存をインストールし、HWA feature は付けない
- 成果物を `sumomo-<tag>-ubuntu-<24.04|26.04>-<x86_64|aarch64>` として `gh release upload` する（sumomo バイナリのみ）
- `slack_notify` の `needs` に `build-sumomo` を追加した
- `README.md` の sumomo 節に GitHub Releases からの入手手順を追記した
