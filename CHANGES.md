...
- [FIX] MP4 の累積再生時間をオーバーフローしない変換で管理するようにする
  ...
  - @sile
- [FIX] WebSocket シグナリング時にサーバー起因の切断で DataChannel の close コールバックが呼ばれないのを修正する
  - 今までは DataChannel シグナリングを利用していない場合、サーバー起因の切断時にオープン中の DataChannel へ close コールバックが通知されず、ユーザー側でチャネルが閉じたことを検知できなかった
  - ユーザーが作成した DataChannel はシグナリング方式に関係なく存在しうるため、Disconnect コマンド経路と同様に close コールバックを通知するようにする
  - @voluntas
- [FIX] native frame 出力で fd 複製に失敗すると不定の fd 番号を要求するのを修正する
  - `F_DUPFD_CLOEXEC` には最低 fd 番号の指定が必須だが、`fcntl` の可変長引数を省略していたため未定義動作になっていた
  - `minfd=0` を明示することで、ゴミ値によるスポラディックな複製失敗を防ぐ
  - @voluntas
- [FIX] AMF エンコーダーで平面高さとフレーム高さが一致しないときに panic せずエラーを返すようにする
  - 今までは `assert_eq!` で panic しており、ドライバが奇数高さなどの平面高さをパディングする場合にプロセスが abort する可能性があった
  - @voluntas
- [FIX] V4L2 デコードでドライバが報告する解像度を `i32` に無検証キャストしていたのを修正する
  - 不正な解像度が負値に切り詰められて `I420Buffer::new` が panic する可能性を防ぐため、`i32::try_from` で検証してから使うようにする
  - @voluntas
- [FIX] OpenH264 エンコーダーの最大空間ビットレート計算で u32 乗算がオーバーフローしないようにする
  - 現実的なビットレートで 2.1 Gbps 超になることはまずありえないが、`saturating_mul` で最大値に丸めることで debug ビルドの panic と release のラップを防ぐ
  - @voluntas
