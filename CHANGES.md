...
- [FIX] MP4 の累積再生時間をオーバーフローしない変換で管理するようにする
  ...
  - @sile
- [FIX] 切断時に DataChannel のクローズ待機中やクローズ完了待機中に `disconnect()` を呼ぶと ack が返らず `Error::CommandResponseMissing` になることがある問題を修正する
  - 切断処理に入った時点でコマンドチャネルを閉じ、以後のコマンド送信は即座に `Error::CommandSendFailed` を返すようにする
  - また、DataChannel の Closing 遷移で誤って close が通知されることがある問題も修正する
  - @melpon
