...
| `--video-codec-implementation` | 任意 (値: `auto` または `internal`/`internal-apple`/`amf`/`nvcodec`/`vpl`/`v4l2`/`openh264` のカンマ区切り) | 映像コーデック実装 (`auto` が既定値) |
...
- `--video-codec-implementation` は `auto` 単独か、`internal`/`internal-apple`/`amf`/`nvcodec`/`vpl`/`v4l2`/`openh264` のカンマ区切りで指定します
