...
# cargo update sora_sdk を実行
def run_cargo_update(dry_run: bool) -> None:
    if dry_run:
        print("Dry-run: Would run 'cargo update sora_sdk'")
    else:
        subprocess.run(["cargo", "update", "sora_sdk"], check=True)
        print("cargo update sora_sdk executed")
